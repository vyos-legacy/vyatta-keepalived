...
void vyatta_if_write_sysctl_value(const char * outpath, int value){
   ...
   fp = fopen(outpath, "w");
   if (!fp){
     printf("couldn't open outpath\n");
     return;
   }
   rc = fprintf(fp, "%d\n", value);
   if (rc < 0)
     printf("couldn't write to path\n");
   printf("Wrote: %d to %s\n", value, outpath);
   fclose(fp);
}
...
void vyatta_if_create_iptables_igmp_filter(const char * ifname){
   ...
   * Since we must listen for multicast traffic on the vmac interface, 
   * because any traffic sourced from the vmac address will be 
   * filtered and not recieved by the parent ethernet interface. 
   ...
   * confuses switch learning and could possibly lead to disruptions we 
   * join on both and filter the outbound IGMP with iptables.
   */
